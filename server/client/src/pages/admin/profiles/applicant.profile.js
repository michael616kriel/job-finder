...
class ApplicantProfile extends Component {
    ...
    async getApplicant() {
        try {
            const response = await axios.get(`${Config.apiUrl}/applicant/get`, {
                params: {
                    uid: this.context.user.uid,
                },
                headers: {
    ...
    async onChange(e) {
        ...
        console.log('SUBMIT', e)
        this.setState({ uploading: true })
        this.fileUpload(e.target.files[0], e.target.name).then((response) => {
            console.log(response.data);
            this.setState({
                uploading: false
            })
                        ...
                        <div className="card profile-card" style={{ 'height': '100%' }}>
                            <div className="card-body text-center">
                                ...
                                <div className="progress mt-2" style={{ 'opacity': (this.state.uploading) ? 1 : 0 }}>
                                    <div className="progress-bar bg-success" role="progressbar" style={{ 'width': `${this.state.uploadProgress}%` }} aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                                <hr />
