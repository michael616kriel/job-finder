...
class Topbar extends Component {
    ...
    async handleFriendRequest(accept: any, uid: any) {
        try {
            const { user } = this.context
            ...
            // console.log('friend-request', response);
            const friendRequests: any = this.state.friendRequests
            for (var k in friendRequests) {
                if (friendRequests[k].uid === uid) {
                    friendRequests.splice(parseInt(k), 1)
                }
            }
    ...
    renderFriendRequests() {
        ...
        for (var key in this.state.friendRequests) {
            const friend: any = this.state.friendRequests[key]
            list.push(
